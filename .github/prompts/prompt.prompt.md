...
2. **Research best practices**:
   ...
   **Model-specific considerations**:
   ...
   _For Model-Agnostic/General Prompts_ (when user says "any" or "general"):
...
6. **Save the prompt** (if requested):
   ...
   Suggest saving to `.github/prompts/` directory alongside this prompt generator:
   - This location is for **generic, reusable prompt tools** only
   - Examples: workflow generators, code review assistants, documentation writers
   - These are tools that can be used across multiple features/projects
   - Use the `.prompt.md` extension for consistency

   Use a descriptive filename:
   - `plan-next-steps.prompt.md` (generic planning tool)
   - `code-review-assistant.prompt.md` (generic review tool)
   - `technical-doc-writer.prompt.md` (generic documentation tool)
   - `email-rewriter-gpt4.prompt.md` (generic writing assistant)

   **Note**: Feature-specific outputs (plans, decisions, specifications) generated BY these prompts should be saved in feature folders (e.g., `.specify/features/F008-hr-interview/planning/`), not here.
