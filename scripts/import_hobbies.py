#!/usr/bin/env python3
"""
Import hobbies from knowledge_base/hobbies/ to EdgeDB.

Hobby has:
- name (Translation)
- tools (optional array<str>)
- article (Translation)
- tags (links)
"""
import edgedb
...
def import_hobbies():
    """Import all hobbies from knowledge_base/hobbies/."""
    hobby_dir = Path('knowledge_base/hobbies')
    
    if not hobby_dir.exists():
        print(f"❌ Directory not found: {hobby_dir}")
        return
    ...
    hobby_files = sorted(hobby_dir.glob('*.md'))
    print(f"\n📁 Found {len(hobby_files)} hobby files\n")
    ...
    for hobby_file in hobby_files:
        try:
            content = hobby_file.read_text()
            frontmatter, body = extract_frontmatter(content)
            
            if not frontmatter:
                print(f"Processing: {hobby_file.stem}")
                print(f"  ⚠️  No frontmatter found, skipping\n")
                skipped += 1
                continue
            
            # Extract fields
            external_id = frontmatter.get('id') or hobby_file.stem
            
            # Check if already exists
            exists = client.query(
                'SELECT Hobby { id } FILTER .external_id = <str>$id LIMIT 1',
                id=external_id
            )
            ...
            # Name (Translation) - check both 'title' and 'name'
            name_data = frontmatter.get('title') or frontmatter.get('name')
            if not name_data:
                print(f"  ❌ Missing title or name\n")
                errors += 1
                continue
            ...
            if tools:
                query_parts.append("tools := <array<str>>$tools,")
