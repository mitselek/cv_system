...
1. **No Emojis:**
   - **Professional documents:** Emojis are strictly forbidden in all professional documents (CVs, motivation letters, application materials).
   - **Technical documentation:** Minimize emoji use in documentation. Use sparingly and only where they add genuine value (e.g., ✅ for completion status, ⚠️ for warnings in user-facing output).
   - **Code & Commits:** No emojis in code, comments, commit messages, or prompts.
   - **Issues & PRs:** Minimize emoji use in GitHub issues, pull requests, and milestones. Use text-based markers (e.g., [DONE], [TODO], CRITICAL) instead.

2. **Type Safety & Code Quality:**
   - **Monitor problems tool:** Always check the problems/errors tool after making code changes. Fix type errors immediately before proceeding.
   - **Type hints:** All Python code must have complete type hints.
   - **Static type checking:** Code must pass Pylance/mypy type checking without errors.
   - **Fix-first policy:** Do not commit code with known type errors or linting issues.

3. **Strict Markdown Linting:** All generated Markdown must be lint-compliant. This includes, but is not limited to:
...
4. **Metadata Headers:** All generated CVs and Cover Letters must begin with an HTML comment header containing the `docID`, `version`, `date`, and `author` for automated processing by the `pdf` script.

5. **Professional Tone:** The language must be professional, clear, and concise.

6. **Recursive Quality:** Any prompt generated by this system must itself include these constitutional rules, ensuring that all standards propagate through all levels of generation.
