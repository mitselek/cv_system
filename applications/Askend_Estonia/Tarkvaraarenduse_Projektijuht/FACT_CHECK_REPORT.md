# Fact-Checking Report: Askend Estonia Tarkvaraarenduse Projektijuht Application

**Date**: 2025-11-22
**Source of Truth**: `/knowledge_base/` modules
**Documents Verified**: CV_Askend_PM.md, motivation_letter_Askend.md

---

## Executive Summary

- **Total Findings**: 0 FABRICATIONS, 0 EMBELLISHMENTS, 0 INCONSISTENCIES, 0 FORMATTING ISSUES
- **Overall Assessment**: VERIFIED - All claims match knowledge base sources

**Result**: Application is integrity-compliant and ready for submission.

---

## Detailed Verification

### Contact Information - VERIFIED ✓

**CV Header:**

- Name: Mihkel Putrinš ✓
- Phone: +372 5656 0978 ✓
- Email: <mitselek@gmail.com> ✓
- GitHub: <https://github.com/mitselek> ✓
- LinkedIn: <https://linkedin.com/in/mitselek> ✓
- Location: Tallinn, Eesti ✓

**Source**: `/knowledge_base/contact.md` - Exact match

### Professional Summary - VERIFIED ✓

**Claim in CV**: "Kogenud IT-projektijuht ja süsteemiarhitekt enam kui 15-aastase kogemusega keerukate IT-süsteemide juurutamisel ja 6+ aastat meeskondade otsest juhtimist"

**Source Verification**:

- 15+ years IT experience: Entusiastid 2010-present (14+ years), plus earlier roles ✓
- 6+ years direct team leadership:
  - EKA IT-osakonna juht 2009-2012 (3 years) ✓
  - PÖFF Arendusjuht 2021-2024 (3 years) ✓
  - Total: 6 years ✓

**Result**: Accurate representation

### Work Experience Entries - ALL VERIFIED ✓

#### Entusiastid OÜ (2010-present)

**Title**: Arhitekt/analüütik/arendaja ✓ (matches source)
**Dates**: 2010 – praegu ✓
**Content**: All achievements match source - 30+ implementations documented in source ✓

#### Ilusa Koodi Instituut (2021-2024)

**Title**: Arendusjuht ✓
**Dates**: 2021 – 2024 ✓
**Scrum methodology claims**: Verified in source body content - "4-liikmelist arendusmeeskonda" ✓
**Achievement**: "Mentordasin neli praktikanti täiskohaga arendajaks (100% püsivusmäär)" ✓
**Source**: achievement `poff-intern-hiring-2024` confirms all 4 interns hired ✓

#### Eesti Kunstiakadeemia (2009-2012)

**Title**: IT-osakonna juhataja ✓
**Dates**: 2009 – 2012 ✓
**All three major achievements** verified against achievements/\*.md files:

- LAN construction (700+ users) ✓ - `eka-lan-construction-2012`
- Email migration (90% cost reduction) ✓ - `eka-email-migration-2012`
- Entu implementation (5000+ assets, 500+ documents) ✓ - `eka-entu-implementation-2012`

#### Eesti Mälu Instituut (2017-2024)

**Title**: Andmekuraator ✓
**Dates**: 2017 – 2024 ✓
**All claims** match source body content ✓
**Achievement**: MySQL adoption by historians ✓ - `mem-historian-db-adoption-2024`

#### Justiitsministeerium (2002-2005)

**Title**: Tarkvaraarendaja ✓ (NOT "projektijuht")
**Language used**: "Osalesin arendusmeeskonna liikmena" ✓ (participated as team member)
**Achievements**: Both match source files:

- `justiitsministeerium-criminal-care-is-2005` ✓
- `justiitsministeerium-criminal-procedure-register-2005` ✓
  **Technologies**: Oracle JDev, Microsoft .NET ✓

**Result**: CORRECT - No leadership language for developer role

#### Other Roles

All verified against source:

- Tartu Ülikool (2014-2015): Teadustarkvara arendaja ✓
- TFTAK (2013-2015): IT-arhitekt ✓
- Soov Kirjastus (1993): Projektijuht (IT) ✓ - `soov-kirjastus-classifieds-db-1993` confirms

### Education Section - VERIFIED ✓

**Secondary Education (1979-1990)**:

- Institutions: Riga Secondary School No. 1, Tallinn Secondary School of Science, Õismäe Humanities Gymnasium ✓
- Degree: Keskharidus ✓

**Higher Education (1990-2002)**:

- Universities: University of Tartu, Tallinn University ✓
- Fields: Matemaatika, Loodusteaduslikud ained, Informaatika ✓
- Status: Lõpetamata kõrgharidus ✓

**CRITICAL CHECK**: Source files `secondary-education-1979-1990.md` and `university-studies-1990-2002.md` contain ONLY metadata, no body content.

**CV education section**: Contains ONLY metadata (institution names, dates, degree status) ✓

**Result**: CORRECT - No fabricated specializations or details added

### Skills Section - VERIFIED ✓

**Project Management & Scrum**:

- 15+ years: Verified above ✓
- 30+ implementations: Documented in Entusiastid source ✓
- Scrum practices: Documented in PÖFF source body content ✓
- All specific skills mentioned exist in knowledge base ✓

**Team Leadership**:

- 4-member team: PÖFF source ✓
- 100% retention: achievement file confirms ✓

**Business Analysis**:

- 30+ organizations: Entusiastid source ✓
- Requirements gathering: Source body content ✓

**Technical Skills**:

- Python (9/10): `skills/python.md` ✓
- JavaScript (9/10): `skills/javascript.md` ✓
- Databases: `skills/database-management.md` ✓
- System architecture: `skills/system-architecture.md` ✓

**Language Skills**:

- Estonian: Native ✓ - `languages/estonian.md`
- English: C2/C1 ✓ - `languages/english.md`
- Russian: B2 ✓ - `languages/russian.md`
- Latvian: C2 ✓ - `languages/latvian.md`

### Certifications - VERIFIED ✓

All 5 certifications match source files exactly:

- MTCIPv6E (2025) ✓
- MTCNA (2025) ✓
- ADO .NET (2003) ✓
- Oracle JDBC (2003) ✓
- UML (2005) ✓

### Hobbies - VERIFIED ✓

All three hobbies exist in knowledge base:

- 3D-modelleerimine ja -printimine ✓
- Astronoomia ja füüsika ✓
- Koorilaul ✓

### Motivation Letter Verification - VERIFIED ✓

**All major claims verified:**

1. "15+ aasta kogemus IT-süsteemide juurutusprojektide juhtimises" ✓
2. "30+ Entu platvormi juurutust" ✓ - source body content
3. "PÖFF 4-liikmelist arendusmeeskonda Scrum/Agile metoodikal" ✓ - source
4. "EKA kampuse LAN-võrgu rajamine (700+ kasutajat)" ✓ - achievement file
5. "E-posti migratsioon (90% kulude kokkuhoid)" ✓ - achievement file
6. "Kõik 4 praktikanti said tööle (100% püsivusmäär)" ✓ - achievement file
7. "Lõpetamata kõrgharidus" ✓ - acknowledged honestly
8. Language proficiency claims ✓ - all match source files

**Gaps section**: Honestly acknowledges incomplete IT degree and MS Excel proficiency gap ✓

**No leadership language** used for Justiitsministeerium developer role ✓

---

## Verification Checklist

- [x] Contact information matches `contact.md` - VERIFIED
- [x] All job titles match source exactly - VERIFIED
- [x] All employment dates match source exactly - VERIFIED
- [x] No leadership language for developer roles - VERIFIED
- [x] All achievements traceable to source files - VERIFIED
- [x] Education section contains ONLY what's in source - VERIFIED
- [x] All skills exist in knowledge base - VERIFIED
- [x] All certifications match source exactly - VERIFIED
- [x] Language proficiency levels accurate - VERIFIED
- [x] All quantified claims (numbers, %) verified - VERIFIED
- [x] No "typical responsibilities" added - VERIFIED
- [x] No role inferences beyond source - VERIFIED
- [x] Motivation letter examples all sourced - VERIFIED

---

## Recommendations

**APPROVED FOR SUBMISSION**

This application demonstrates:

1. **Perfect integrity** - zero fabrications or embellishments
2. **Honest gap acknowledgment** - incomplete IT degree disclosed
3. **Conservative interpretation** - no additions beyond source
4. **Proper role language** - developer roles use "participated", leader roles use "led"
5. **Accurate quantification** - all numbers match source data

**No corrections needed.** Application is ready for Estonian grammar correction and PDF generation.

---

## Source Files Referenced

- `/knowledge_base/_compiled_context.md`
- `/knowledge_base/contact.md`
- `/knowledge_base/experiences/entusiastid-ou-2010-present.md`
- `/knowledge_base/experiences/ilusa-koodi-instituut-2021-2024.md`
- `/knowledge_base/experiences/eesti-kunstakadeemia-2009-2012.md`
- `/knowledge_base/experiences/eesti-malu-instituut-2017-2024.md`
- `/knowledge_base/experiences/justiitsministeerium-2002-2005.md`
- `/knowledge_base/experiences/tartu-ulikool-2014-2015.md`
- `/knowledge_base/experiences/tftak-2013-2015.md`
- `/knowledge_base/experiences/soov-kirjastus-1993.md`
- `/knowledge_base/achievements/*.md` (all relevant files)
- `/knowledge_base/education/*.md`
- `/knowledge_base/skills/*.md`
- `/knowledge_base/certifications/*.md`
- `/knowledge_base/languages/*.md`
- `/knowledge_base/hobbies/*.md`
